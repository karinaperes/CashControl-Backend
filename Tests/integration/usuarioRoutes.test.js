...
describe('Testes de Integração - UsuarioController', () => {
    ...
    describe('DELETE /usuario/:id', () => {
        it('deve excluir um usuário específico', async () => {
            const response = await request(app).delete(`/usuario/${usuarioCriado.id}`);

            expect(response.status).toBe(200);
            expect(response.body).toHaveProperty('mensagem', 'Usuário excluído com sucesso!');

            // Verifica se o usuário foi realmente excluído
            const verificaUsuario = await request(app).get(`/usuario/${usuarioCriado.id}`);
